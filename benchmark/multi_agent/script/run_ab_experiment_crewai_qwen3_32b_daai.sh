...
CREWAI_FAULT_SCRIPT="${SCRIPT_DIR}/run_crewai_fault_injection_qwen3_32b_daai.sh"
...
# Qwen3-32B PP2+DP2 on A100 needs host HiCache larger than the device KV pool.
HICACHE_SIZE=40
QUANTIZATION=""
...
TP_SIZE=1
NNODES=1
TARGET_NODE="${TARGET_NODE:-hkbugpusrv07}"
MODEL_LOCAL_ROOT="${MODEL_LOCAL_ROOT:-/tmp/${USER}/sglang-model-cache}"
VENV_LOCAL_ROOT="${VENV_LOCAL_ROOT:-/tmp/${USER}/sglang-runtime-cache}"
FAULT_DP_RANK="0"
FAULT_PP_RANK="0"
FAULT_TP_RANK="0"
...
usage() {
    cat <<'EOF'
Usage:
  ./run_ab_experiment_crewai_qwen3_32b_daai.sh [options]
...
Options:
  ...
  --target-node NODE         Pin the SLURM job to one node (default: hkbugpusrv07)
  ...
  --venv-local-root DIR      Local runtime cache root on the target node
...
run_experiment() {
    ...
    log "  venv-local-root:  ${VENV_LOCAL_ROOT}"
    ...
    local sbatch_args=(
        ...
        "${SLURM_DIR}/run_server_qwen3_32b_daai.slurm"
        --dp-size "${DP_SIZE}"
        --pp-size "${PP_SIZE}"
        --tp-size "${TP_SIZE}"
        --model-path "${MODEL_PATH}"
        --model-local-root "${MODEL_LOCAL_ROOT}"
        --venv-local-root "${VENV_LOCAL_ROOT}"
