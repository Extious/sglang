#!/bin/bash
# 2-Group Fault Tolerance Experiment: Qwen3-8B on A100 GPUs.
#
...
#
# Port allocation (avoids conflict with 4B and 32B experiments):
#   Workers:     28000-28001
#   Peer cache:  29000-29001
#   Router:      50000
#
# Usage:
#   ./run_ab_experiment_qwen3_8b.sh [options]
#
# Options:
...
LOG_DIR="${MULTI_AGENT_DIR}/logs_qwen3_8b"
...
QUANTIZATION=""
ROUTER_PORT=50000
PROMETHEUS_PORT=59000
ROUTER_HOST="127.0.0.1"
WORKER_BASE_PORT=28000
PEER_PORT_BASE=29000
OUTPUT_BASE_DIR="${MULTI_AGENT_DIR}/logs_qwen3_8b/agent_workspace/timing_reports_qwen3_8b"
...
EXPERIMENT_LOG="${LOG_DIR}/ab_experiment_8b_${TIMESTAMP}.log"
mkdir -p "${LOG_DIR}"
...
kill_worker_ports() {
    local base_port="${1:-28000}"
    local num_workers="${2:-2}"
    local peer_port_base="${3:-29000}"
...
build_worker_restart_cmd() {
    ...
    echo "${venv_activate}PYTHONPATH=${REPO_ROOT}/python:${REPO_ROOT}:\${PYTHONPATH:-} exec python -m sglang.launch_server --model-path ${MODEL_PATH} --tp 1 --host 0.0.0.0 --port {PORT} --enable-metrics --attention-backend triton --sampling-backend pytorch --tool-call-parser qwen --enable-cache-report --enable-hierarchical-cache --hicache-size ${HICACHE_SIZE} ${quant_flag}2>&1"
}
...
run_experiment() {
    ...
    local sbatch_args=(
        --nodes=1
        "${SLURM_DIR}/run_server_qwen3_8b.slurm"
    ...
    local router_args=(
        ...
        --chunk-timeout-secs 20
        --prefill-timeout-secs 60
...
log "============================================================"
log "4-Group Fault Tolerance Experiment — Qwen3-8B on A100"
log "============================================================"
...
log ""
log "============================================================"
log "2-Group Experiment Complete — Qwen3-8B"
log "============================================================"
