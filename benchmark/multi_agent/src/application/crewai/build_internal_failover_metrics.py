...
def build_trace_peer_cache_hits(
    ...
    for record in trace_records:
        ...
        for timing in record.get("agent_timings", []):
            ...
            peer_hit_tokens = int(timing.get("peer_buffer_cached_tokens", 0) or 0)
            if peer_hit_tokens <= 0:
                continue
            per_job_tokens[job_id] += peer_hit_tokens
            if task_label:
                per_task_tokens[(job_id, task_label)] += peer_hit_tokens
...
def build_jobs_output(
    ...
    peer_cache_hits_by_job: dict[str, int],
) -> list[dict[str, Any]]:
        ...
        jobs.append(
            {
                ...
                "peer_cache_hit_tokens": int(peer_cache_hits_by_job.get(job_id, 0) or 0),
...
def build_tasks_output(
    ...
    rid_summary: dict[str, dict[str, Any]],
    impacted_job_ids: set[str],
    peer_cache_hits_by_task: dict[tuple[str, str], int],
) -> list[dict[str, Any]]:
    ...
    for record in trace_records:
        job_id = str(record.get("topic_id", ""))
        if job_id not in impacted_job_ids and not any(
            key[0] == job_id for key in peer_cache_hits_by_task
        ):
            continue
        for timing in record.get("agent_timings", []):
            agent_name = str(timing.get("agent", "") or "")
            ...
            task_summary["peer_cache_hit_tokens"] = int(
                peer_cache_hits_by_task.get((job_id, task_label), 0) or 0
            )
    ...
    for task_summary in per_task.values():
        ...
        hit_tokens = int(task_summary.get("peer_cache_hit_tokens", 0) or 0)
...
def main() -> None:
    ...
    (
        peer_cache_hits_by_job,
        peer_cache_hits_by_task,
    ) = build_trace_peer_cache_hits(trace_records)
