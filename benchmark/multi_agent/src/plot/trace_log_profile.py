...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input",
        type=Path,
        default=_SRC_ROOT / "crewai" / "trace_log.json",
        help="Path to the input trace log JSON file.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=_SRC_ROOT / "crewai" / "trace_log_profile.png",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to job_summary.csv.",
    )
    parser.add_argument(
        "--task-summary",
        type=Path,
        default=None,
        help="Path to task_summary.csv.",
    )
...
def _float_value(value: str | None):
    if value in (None, ""):
        return float("nan")
    try:
...
def load_records_from_summaries(job_summary: Path, task_summary: Path) -> list[dict]:
    jobs = _load_csv_rows(job_summary)
    tasks = _load_csv_rows(task_summary)
    tasks_by_job: dict[str, list[dict[str, str]]] = {}
    for row in tasks:
        tasks_by_job.setdefault(str(row.get("job_id", "")), []).append(row)
    ...
    for job in jobs:
        ...
        for task in tasks_by_job.get(job_id, []):
            prompt = _int_value(task.get("prompt_tokens"))
            cached = _int_value(task.get("cached_prompt_tokens"))
            completion = _int_value(task.get("completion_tokens"))
            timings.append({
                "agent": task.get("agent", ""),
                "task_label": task.get("task_label", ""),
                "start": None,
                "end": None,
                "duration_s": _float_value(task.get("task_completion_time_s")),
                "prompt_tokens": prompt,
                "cached_prompt_tokens": cached,
                "backup_cache_tokens": 0,
                "completion_tokens": completion,
                "total_tokens": _int_value(task.get("total_tokens")) or prompt + completion,
                "llm_calls": _int_value(task.get("llm_calls")),
                "prefill_cache_hit_rate": (
                    round(cached / prompt, 4) if prompt else None
                ),
                **({"error": task.get("error")} if task.get("error") else {}),
            })
        records.append({
            "topic_id": job_id,
            "topic": job.get("job", ""),
            ...
            "summary": {
                "wall_time_s": _float_value(job.get("job_completion_time_s")),
                "llm_calls": _int_value(job.get("llm_calls")),
                "prompt_tokens": sum(t["prompt_tokens"] for t in timings),
                "prefill_cached_tokens": _int_value(job.get("prefill_cached_tokens")),
                "total_tokens": _int_value(job.get("total_tokens")),
            },
        })
    return sorted(records, key=topic_sort_key)
...
def build_series(records: list[dict], agent_names: list[str]) -> tuple[list[str], dict, list[float]]:
    ...
    has_any_timing = any(record.get("agent_timings") for record in records)
    ...
    for record in records:
        ...
        if has_any_timing:
            ...
            total_prompt = summary.get("prompt_tokens", 0)
            total_tokens = summary.get("total_tokens", 0)
            wall_time = summary.get("wall_time_s", 0)
            completion = max(0, total_tokens - total_prompt)
            n_agents = len(agent_names)
            for i, agent in enumerate(agent_names):
                if i == 0:
                    metrics[agent]["prompt_tokens"].append(total_prompt)
                    metrics[agent]["completion_tokens"].append(completion)
                    metrics[agent]["duration_s"].append(wall_time)
                else:
                    metrics[agent]["prompt_tokens"].append(float("nan"))
                    metrics[agent]["completion_tokens"].append(float("nan"))
                    metrics[agent]["duration_s"].append(float("nan"))
...
def plot_chart(
    ...
    *,
    ...
    main_title = "CrewAI Runtime and Token Profiling"
    if subtitle:
...
def main() -> None:
    args = parse_args()
    if args.job_summary and args.task_summary:
        records = load_records_from_summaries(args.job_summary, args.task_summary)
    else:
    ...
    if records:
        r0 = records[0]
        parts = []
        em = r0.get("experiment_mode")
        st = r0.get("server_topology")
        if em:
            parts.append(str(em))
        if st:
            parts.append(str(st))
